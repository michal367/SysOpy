...
void print_process_usage(struct rusage* usage){
    printf("\nProces PID %d usage: \n", (int)getpid());
...
void start(int mode, int p_amount, int p_sec, int cpu_time_sec, int memory_MB){
    ...
    for(int i=0; i < p_amount; i++){
        ...
        if(pids[i] < 0){
            ...
            rl_cpu.rlim_cur = cpu_time_sec;
            rl_cpu.rlim_max = cpu_time_sec;
            setrlimit(RLIMIT_CPU, &rl_cpu);
            // setting memory limit
            struct rlimit rl_memory;
            getrlimit(RLIMIT_AS, &rl_memory);
            rl_memory.rlim_cur = memory_MB;
            rl_memory.rlim_max = memory_MB;
            setrlimit(RLIMIT_AS, &rl_memory);
...
void help(){
    printf("macierz list_file proc_amount sec mode cpu_time_sec memory_MB\n");
    ...
    printf("cpu_time_sec - limit of cpu usage in seconds\n");
    printf("memory_MB - limit of memory usage in MB\n");
}
