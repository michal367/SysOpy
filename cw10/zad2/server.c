...
void get_message(int socket){
    ...
    if(msg.type == MSG_NAME){
        ...
        if(user_id == -1){
            sendto_msg(socket, &addr, addr_len, MSG_NAME, 0, "No free slot on server");
            return;
        }
        printf("New user, name: %s\n", msg.msg);
        int free_id = get_free_user(&users, user_id);
        if(free_id == -1){
            sendto_msg(socket, &addr, addr_len, MSG_NAME, user_id, "Wait for opponent");
        }
        else{ // game starts
...
void* thread_ping(void* val){
    ...
    while(1){
        ...
        for(int i=0; i < users.size; i++){
            ...
            sendto_msg(users.users[i].socket, users.users[i].addr, users.users[i].addr_size, MSG_PING, 0, "");
